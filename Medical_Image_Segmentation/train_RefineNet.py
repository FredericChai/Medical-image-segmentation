...
from unet import UNet,UNet1,ResNetUNet,rf101
from utils import *
import torch.nn.functional as F

#import vnet
# from torchfcn.models import FCN16s
...
def get_args():
	...
	parser.add_option('--result_path',default= '/mnt/HDD1/Frederic/Segmentation/Seg_refine/result/',type = str,help='control the mode of optimizer')
...
def train_net(net,
			  ...
			  gpu=True,
			  target_path = '',
			  checkpoint_path='/mnt/HDD1/Frederic/Segmentation/Seg_refine/checkpoints/',
			  result_path = '/mnt/HDD1/Frederic/Segmentation/Seg_refine/result/'):
...
#Train iteration
	logger = Logger(result_path+'log.txt',title = 'V3')
	...
	for epoch in range(start_epoch,start_epoch + epochs):
		...
		with tqdm(total = datasize/batch_size) as pbar:
			for ite,data in enumerate(trainloader[0]):
				...
				masks_pred = F.interpolate(masks_pred,size=[512,512],mode = 'bicubic')
		...
		val_dice = eval_net(net, val_sets,epoch,args.gpu_id,save_sample_mask,result_path)
...
if __name__ == '__main__':
	...
	net = rf101(num_classes = 2,pretrained = True)
	# net = vnet.VNet(elu=False, nll=nll)
	# net = FCN16s(n_class = 2)
...
#Use checkpoint
	...
	if args.test:
		...
		best_checkpoint = '/mnt/HDD1/Frederic/Segmentation/Seg_refine/checkpoints/best_checkpoint.pth'
		test_net(net = net,
				dataset = testloader,
